...
class Brain_To_Server(Dataset):
    def __init__(self, options, mode='train'):
        ...
        self.cts = sorted(os.listdir(self.data_dir), key=natural_sort_key)
        self.labels = sorted(os.listdir(self.label_dir), key=natural_sort_key)
    ...
    def __getitem__(self, idx):
        image_name = self.cts[idx]
        label_name = self.labels[idx]

        image = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(self.data_dir,image_name)))
        label = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(self.label_dir,label_name)))
...
class Brain_To_Server_Tif(Dataset):
    def __init__(self, options, mode='train'):
        print('Get 15passes/30passes for image-to-image mapping. all needed.')
        self.mode = mode
        assert mode in ['train', 'val'], f'{mode} is not in [train/val]'
    ...
    def __getitem__(self, idx):
        ...
        image = imreadTiff(os.path.join(self.data_dir, image_name))
        label = imreadTiff(os.path.join(self.label_dir,label_name))
