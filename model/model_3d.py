...
class DeConvBnRelu3d(nn.Module):
    def __init__(self, in_chl, out_chl, kernel_size=(2, 1, 1), dilation=1, stride=2, groups=1, chls_group=12, is_bn=True,
                 is_relu=True):
        super(DeConvBnRelu3d, self).__init__()
        self.conv = nn.ConvTranspose3d(in_chl, out_chl, kernel_size=kernel_size, padding=(1, 0, 0), stride=(2, 1, 1),
                              dilation=dilation, groups=groups, bias=True)
        self.bn = None
        self.relu = None
...
class StackResEncoder3d(nn.Module):
    def __init__(self, in_chl, out_chl, kernel_size=3):
        ...
        if in_chl != out_chl:
            self.convx = ConvBnRelu3d(in_chl, out_chl, kernel_size=1, dilation=1, stride=1, groups=1, is_bn=False,
                                      is_relu=False)
...
class StackResDecoder3d(nn.Module):
    def __init__(self, in_chl, out_chl, kernel_size=3):
        ...
        self.conv2 = ConvBnRelu3d(out_chl, out_chl, kernel_size=kernel_size, dilation=1, stride=1, groups=1,
                                  is_relu=False)
        self.convx = ConvBnRelu3d(in_chl + out_chl, out_chl, kernel_size=1, dilation=1, stride=1, groups=1, is_bn=False,
                                  is_relu=False)
...
class UNet3d(nn.Module):
    def __init__(self, in_chl=1, out_chl=1, model_chl=32):
        ...
        self.end = nn.Sequential(
            DeConvBnRelu3d(self.model_chl, self.out_chl, kernel_size=(2, 1, 1), stride=(2, 1, 1), is_bn=False, is_relu=False))
...
class ResUNet3d(nn.Module):
    def __init__(self, in_chl=1, out_chl=1, model_chl=32):
        ...
        self.end = nn.Sequential(
            ConvBnRelu3d(self.model_chl, self.out_chl, kernel_size=1, stride=1, is_bn=False, is_relu=False))
...
class StackDenseEncoder3d(nn.Module):
    def __init__(self, in_chl, out_chl, kernel_size=3):
        ...
        self.conv3 = ConvBnRelu3d(in_chl + out_chl + out_chl, out_chl, kernel_size=kernel_size, dilation=1, stride=1,
...
if __name__ == '__main__':
    x = torch.rand([2, 1, 16, 512, 512]).cuda()
    model = UNet3d(in_chl=1, out_chl=1).cuda()
    y = model(x)
    print(y.shape)